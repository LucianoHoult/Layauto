...
**Resize placement model decision (from unmerged `claude/pensive-cray-j0y46y`).** Stage 5 needs an explicit placement model before its transaction semantics are rebuilt. Adopt **Model A: fixed cell frame + static FIN + OD-driven resize**. The standard-cell row frame, VSS/VDD rails, whole FIN grid, rail-side gate endpoints, NWELL, BOUNDARY, and M1 rail locations stay fixed for an in-row drive-strength ECO; `nfin` shrink/grow changes the active OD coverage from the **gap side** of the device. Anchor direction is derived from geometry (rail proximity), not from `dev_type` strings. This rejects the current dummy target's shrink-to-fit interpretation, where PMOS fins/rail/frame move downward, because variable-height rows contradict the project scope and standard-cell placement model.

The unmerged branch measured the current `MN0:5→4` / `MP0:7→6` output against the from-scratch target and found a three-way inconsistency: current Stage 5 removes the top fin from both devices, recomputes part of the frame (`cell_height`, NWELL/BOUNDARY), but leaves M1 rails and the N/P gap effectively fixed. That is neither fixed-frame nor shrink-to-fit. Under Model A, NMOS drops the top/gap-side active fin while PMOS drops the bottom/gap-side active fin; FIN itself is unchanged; POLY / NWELL / BOUNDARY / M1 / rails / cell height are reused on shrink. The expected edited surface becomes OD plus any gap-side, non-via-anchored LI repair, with grow support gated by CSP. Regenerate the fixture target under Model A and then make the target comparison a hard regression gate instead of a printed mismatch summary.
