...
2026-09-09 用户说明，生产服务器隔离，含实际 Calibre 版本与保密 PDK；只能单向向内上传代码，向外仅能描述文件数值脱敏后的格式。这不是可供 agent 直接 SSH/cloud 访问的开发主机，不应用上文一般远程连接流程。生产文件、PDK、日志和许可证不假定可复制到本机或 GitHub。

具体 OS/CPU/Calibre build、Python/依赖离线可用性，以及能否反馈公共代码版本、测试编号和通过/失败类别尚待确认；不能从“能上传代码”推断能联网安装包或传出结果。M1 样本范围与隔离验证路径建议见 [W003](../tasks/W003-m1-fixture-contracts.md)，真实验收义务继续以架构为准。本轮未连接服务器或安装 VM/EDA。
