...
**Last verified:** 2026-04-28 on branch `claude/m5-drc-derivator` (post-M5).
...
**M4 + M5 are complete.** The next move is **M6** — the L3 macro family. With M2's L2 primitives + engine transactions, M3's shape_pool, M4's B-tier cell-grid + net-equivalence + L2 atomics, and M5's C1 derivator all in place, M6 adds the device / net / diffusion / cut macros that validate the four-layer architecture beyond resize:
1. **New** `core/macros/{device_add,device_remove,net_reroute,buffer_insert,share_diffusion,split_diffusion,cut_ops}.py` — each macro brackets its work in `engine.checkpoint` / `commit_with_full_delta` and returns L1 EditOps.
2. **`share_diffusion(dev_a.pin, dev_b.pin)`** lights up M4d's `mark_shared_diffusion` against engine cells (now possible because M4e added OD to the engine). Its `split_diffusion` inverse needs to add a `CPO` cut first via `add_cut_cell` to avoid a transient short.
3. **`add_cut` / `remove_cut`** call `add_cut_cell` / `remove_cut_cell`; the engine's net-equivalence union-find recomputes the affected component automatically (the §B "no CUT between adjacent cells" rule fires through `union`).
4. **Decoder rejection of direct edits to derived shapes** — the M5 `is_derived` seam is set on every C1 shape post-derivator; M6 adds the rejection check at the decoder's Phase 1 entry so a future macro that tries to modify NWELL/BOUNDARY directly fails loud.
5. M6 also flips the L3 resize macro from `commit_with_delta` to `commit_with_full_delta` so the derivator can subscribe to per-cell-delta affected-neighborhood updates instead of full recompute.
