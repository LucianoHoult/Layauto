...
**Last verified:** 2026-04-27 on branch `claude/review-arch-plan-Niwe4` (post-M4b).
...
- **Status:** [~] In progress (M4a Done 2026-04-27; M4b Done 2026-04-27; M4c–e pending)
- **Owner:** Claude (`claude/review-arch-plan-Niwe4`) for M4a + M4b; M4c–e _unassigned_
...
- **Files touched.**
  ...
  - `tech/layer_map.py` (M4a) — tier markers + helpers; M4c/d/e will add cut-layer GDS numbers as fixtures grow them.
- **Change outline.**
...
- **Notes for downstream M4 sub-milestones.**
  ...
  - **M4b ↦ M4c.** `MultiLayerGrid.bbox_to_b_tier_cells` is the projection M4c calls. M4c supplies `(axis_a_layer, axis_b_layer)` per B-tier layer (OD: POLY × FIN; VIA0: LI × M1; CPO: POLY × FIN; etc.) via `register_b_tier_axes`. The parser stamps each projected cell as a `CellOccupancy` and, for OD cells between adjacent gates, walks the device list to set `owner_device_id` and append to `shared_with`.
  - **M4b ↦ M4d.** The L2 op `mark_shared_diffusion(dev_a, dev_b)` calls `engine.union` on the OD cell pairs that connect the two devices' diffusions. The §B "no CUT between adjacent cells" rule then naturally manifests because a CUT cell on the chain fails the union step. The L2 ops `add_cut_cell` / `remove_cut_cell` go through `engine.mark_cut` (and a future inverse).
...
M1, M2, M3, M4a, and M4b have landed. The next move is **M4c** — invert the parser's tier dispatch so FIN / POLY / OD / VIA0 / CUT layers route through `tech.layer_map.tier_of`. Concretely: for every B-tier `ShapeRecord` in the post-M3 `shape_pool`, the parser calls `MultiLayerGrid.bbox_to_b_tier_cells` with the appropriate `(axis_a_layer, axis_b_layer)` pair (OD: POLY × FIN; VIA0: LI × M1; CPO: POLY × FIN), stamps a `CellOccupancy` per cell via `set_b_tier_cell`, and (for OD cells between adjacent gates) walks the device list to set `owner_device_id` plus append to `shared_with`. The M4b cell-grid storage and the M4a `CellOccupancy` validator are both ready to receive these stamps. The dummy fixture has no CUT shapes today, so `add_cut_cell` exercise lands as M4d when fixtures grow. The retirement target on the macro side is the `device_y_marker` desc-substring filter inside `core/solver.py:362-377` — once OD cells carry `owner_device_id`, that string match retires.
