...
**Last verified:** 2026-04-28 on branch `claude/review-arch-plan-Niwe4` (post-M4c).
...
- **Status:** [~] In progress (M4a Done 2026-04-27; M4b Done 2026-04-27; M4c Done 2026-04-28; M4d–e pending)
- **Owner:** Claude (`claude/review-arch-plan-Niwe4`) for M4a + M4b + M4c; M4d–e _unassigned_
...
M1, M2, M3, M4a, M4b, and M4c have landed. The next move is **M4d** — promote the B-tier L2 atomics to the same status the LI-modify primitives have today. Concretely: add `extend_od`, `extend_poly`, `add_fin_strip` / `remove_fin_strip`, `add_cut_cell` / `remove_cut_cell`, and `mark_shared_diffusion` to `core/atomic_ops.py`. They consume the M4b cell-grid (`grid.b_tier_cells_of`, `grid.set_b_tier_cell`) and call `engine.propose_assign` / `engine.mark_cut` / `engine.union` against the engine's CSP cells. The macro side is `core/solver.py::_emit_fin_removes` / `_emit_od_modify` / `_emit_poly_modify_if_endpoint_changed` — these still emit L1 records directly (non-CSP side-channel) and are the eviction targets: once their L2 equivalents land, the macro brackets them inside the existing checkpoint/commit transaction and the decoder synthesises L1 from the cell delta. The dummy fixture's diffusion-share / cut paths aren't exercised yet (single NMOS + single PMOS, no CPO), so M4d should ship with synthetic test fixtures à la `test_blockage` and `test_parser_tier_dispatch::test_project_records_shared_with_when_two_devices_overlap`. M4d also retires the legacy desc-substring fallback in `_reshape_li_sd_bars` once it ports any remaining direct-construction callers (today: ad-hoc test scaffolding only).
