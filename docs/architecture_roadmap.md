...
**Last verified:** 2026-04-28 on branch `claude/review-arch-plan-Niwe4` (post-M4e — M4 complete).
...
**M4 is complete** (M4a–M4e all merged). The next move is **M5** — pull all C1 derived markings (NWELL, VT, PP, NP, BOUNDARY, DNW) out of `pipeline/run_mvp.py` hardcoded loops + the decoder's `_derive_*` Phase 2 helpers and into a new `core/drc_derivator.py` that subscribes to CSP commit deltas. The M4 commit-delta plumbing is ready: `engine.commit_with_full_delta` returns `CommitDelta(cells, unions)` so the derivator can recompute affected C1 shapes from the cell delta + Device metadata. Concretely:
1. **New** `core/drc_derivator.py` with a `DRCDerivator(grid, model, config)` class. Inputs: CSP `CommitDelta` + `Device` metadata + a rule table. Output: L1 `EditOp`s through the M1 decoder.
2. **Move** the transitional `_derive_nwell` + `_derive_boundary` helpers from `core/decoder.py::WritebackDecoder` into the derivator. The decoder's Phase 1 path consumes the derivator's L1 EditOps unchanged.
3. Subscription granularity: per-cell-delta affected neighborhood. Local recomputation, not full rebuild. The "affected neighborhood" radius is per-rule.
4. **`ShapeRecord.is_derived`** (M3 seam) gets set to `True` for every C1 shape the derivator emits. The decoder rejects direct edits to derived shapes.
5. C2 markers (DIODE / ESD / TEXT) are NOT in the derivator — they go through the M3 `shape_pool` direct-edit path the macros already use.
