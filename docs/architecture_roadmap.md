...
**Last verified:** 2026-04-28 on branch `claude/review-arch-plan-Niwe4` (post-M4d).
...
- **Status:** [~] In progress (M4a Done 2026-04-27; M4b Done 2026-04-27; M4c Done 2026-04-28; M4d Done 2026-04-28; M4e pending)
- **Owner:** Claude (`claude/review-arch-plan-Niwe4`) for M4a–M4d; M4e _unassigned_
...
- **Acceptance.**
  ...
  - **M4e.** Cell-level DRC rules for OD / VIA0; CUT-to-gate, CUT-to-via, CUT-to-CUT spacings; performance instrumentation on `_propagate`.
...
M1, M2, M3, M4a, M4b, M4c, and M4d have landed. The next move is **M4e** — cell-level DRC rules for B-tier layers + performance instrumentation. Concretely:
1. Extend `core/drc_constraints.py` with rules that fire on B-tier cells: OD-to-OD spacing (when not shared), VIA0 enclosure inside LI / M1, CUT-to-gate / CUT-to-via / CUT-to-CUT spacings. The M4d L2 atomics (`add_cut_cell`, `mark_shared_diffusion`, `extend_od`) provide the cell-grid surface those rules read.
2. Add OD / VIA0 to the engine via `engine.add_layer` so the cell-level rules have something to fire on. The M4b `MultiLayerGrid.bbox_to_b_tier_cells` projection gives the bounds.
3. Performance instrumentation on `ConstraintEngine._propagate` per-layer × track-count — the roadmap §M4 risk note calls out this as the first place engine complexity becomes visible. Add a minimal stopwatch helper that logs `propagate(layer)` durations on each `assign`/`union` call.
4. Once OD is in the engine, M4d's `mark_shared_diffusion` and `add_cut_cell` start firing union/cut events on real cells — the M4e PR should include an acceptance test that proves a shared-diffusion union landing on OD cells holds across DRC propagation.
