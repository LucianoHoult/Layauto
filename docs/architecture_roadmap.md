...
**Last verified:** 2026-04-29 on branch `claude/review-arch-plan-fk4Og` (post-M6a).
...
| 13 | ~~No `core/macros/` directory; no `pick_macro` dispatch~~ | M6a created `core/macros/` with `cut_ops.py` (`add_cut` / `remove_cut`) and `share_diffusion.py` (`share_diffusion`), each bracketing L2 calls in `engine.checkpoint` / `commit_with_full_delta`. `resize_device` remains in `core/solver.py` for backward compatibility; M6b `pick_macro` dispatch table will lift it into `core/macros/` alongside the device-add / net-reroute / split_diffusion family. | half-built (M6a created the directory; M6b lights up the dispatch + remaining macros) |
...
- **Status:** [ ] In progress (M6a Done 2026-04-29)
- **Owner:** Claude (`claude/review-arch-plan-fk4Og`) for M6a
...
- **Sub-milestones.**
  ...
  - **M6b (Not started).** Remaining macro families + `pick_macro` dispatch.
    - **New** `core/macros/{device_add,device_remove,net_reroute,buffer_insert,split_diffusion}.py`.
    - `core/diff.py` — `diff_cdl` extends to device add/remove + diffusion-share delta + cut delta.
    - `pipeline/run_mvp.py` — `pick_macro` dispatch table grows.
    - Acceptance scenarios listed below.
- **Files touched (cumulative).**
  ...
  - **M6b:** **new** `core/macros/{device_add,device_remove,net_reroute,buffer_insert,split_diffusion}.py`, `core/diff.py` (extended), `pipeline/run_mvp.py` (`pick_macro`).
- **Change outline.**
  ...
  - `split_diffusion(...)` — inverse; if devices are physically adjacent, must call `add_cut_cell` first to ensure isolation. (M6b.)
  - `add_cut` / `remove_cut` — direct calls to `add_cut_cell` / `remove_cut_cell`; CSP recomputes net-equivalence. (M6a done.)
- **Acceptance (M6 milestone-level — M6b adds).**
  ...
  - "Two devices request shared S/D" → `share_diffusion` macro lands legally. (M6a smoke-test passes; M6b adds full L1 emission.)
  - "Same poly must be cut" → `add_cut` macro inserts a CPO mid-span and splits the net. (M6a smoke-test passes; M6b adds the GDS layer-number entry.)
...
- **Notes for downstream sub-milestones.**
  - The M6a `CutMacroResult.commit_delta` is typically empty for `add_cut` because `engine.mark_cut` pins the cell directly (bypassing the trail). The structural change lives on `cell.fixed` / `cell.assignment`. Consumers (M6b L1 synthesis) read the macro result, not the cell delta.
  - M6a's `share_diffusion` `cells_unioned` count and `commit_delta.unions` length differ: the L2 atomic counts every `engine.union` True return (including no-op successes when the cells are already in the same component), while `commit_delta.unions` only carries actual merges. M6b should pick the right count for its purpose (display vs. L1 emission).
  - The M5 derivator's exempt-prefix convention (`desc='derived_<layer>_y2_shift'`) is the M6a contract for derived-shape edits. M6b derivator extensions (VT / PP / NP / DNW) must keep the prefix, or extend the exempt check.
...
**M6a is complete.** The next move is **M6b** — extend the macro family to the device / net / diffusion-split families that complete the §C four-layer split, plus a `pick_macro` dispatch table that routes `diff_cdl` deltas to the right macro:

1. **New** `core/macros/{device_add,device_remove,net_reroute,buffer_insert,split_diffusion}.py` — each macro brackets its work in `engine.checkpoint` / `commit_with_full_delta` and returns L1 `EditOp`s. The result type pattern (success / commit_delta / detail) follows M6a's `CutMacroResult` / `ShareDiffusionResult`.
2. **`split_diffusion(dev_a.pin, dev_b.pin)`** must call M6a's `add_cut` first (CPO between the gates) before walking `mark_shared_diffusion`'s inverse, otherwise it produces a transient short circuit. This is the place §C's "engine transactions are how M6 stays correct" gets stress-tested.
3. **`pick_macro` dispatch table** in `pipeline/run_mvp.py` routes `diff_cdl`'s output to one of the macro implementations. `diff_cdl` itself extends to detect device-add / device-remove / shared-diffusion / cut deltas in the CDL pair (today it only handles parameter-level fin counts).
4. **L1 emission for cut layers** — once `tech/layer_map.py` has GDS-number entries for `CPO` / `M0_CUT` / `FIN_CUT`, M6a's `add_cut` macro starts emitting `add_shape` / `remove_shape` records the decoder can apply. The macro result already carries the cell-grid mutation; the L1 step is the GDS-side hand-off.

After M6b, the 7-milestone roadmap closes with **M7** — Virtuoso SKILL + Calibre DRC/LVS closure (real PDK environment).
