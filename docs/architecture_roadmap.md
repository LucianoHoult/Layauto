...
**Last verified:** 2026-04-27 on branch `claude/review-arch-plan-Niwe4` (post-M4a).
...
| 9 | `MultiLayerGrid` is 1D-track only; no `CellOccupancy` *cell-grid wiring* | `core/grid.py:87-240`; M4a defined `core/data_model.py::CellOccupancy` (B-tier 2D record with `owner_device_id` + `shared_with[]` + `shape_record` backlink) but the grid still stores only `TrackSegment`s. | half-built (M4a added the type; M4b adds the cell-grid axis) |
...
- **Status:** [~] In progress (M4a Done 2026-04-27; M4b–e pending)
- **Owner:** Claude (`claude/review-arch-plan-Niwe4`) for M4a; M4b–e _unassigned_
...
- **Sub-milestones.**
  ...
  - **M4b (Not started).** CSP cell-grid axis + net-equivalence union-find. `MultiLayerGrid` gains a 2D `(track_a, track_b)` cell-grid for B-tier layers; engine grows `net_of(cell)` / `connected_cells(net_id)` plus a `union` precondition "no CUT between adjacent cells"; `commit_with_delta` extends to also export a net-equivalence delta.
  - **M4c (Not started).** Parser tier-dispatch via `tech.layer_map.tier_of`. FIN / POLY / OD / VIA0 / CUT route to the right projection path; OD shapes between adjacent gates are flagged as shared-diffusion candidates; `CellOccupancy.owner_device_id` + `shared_with[]` get stamped from gate-position cross-reference. Replaces the `device_y_marker` desc-substring filter inside `core/solver.py:362-377`.
  - **M4d (Not started).** B-tier L2 atomics. `extend_od`, `extend_poly`, `add/remove_fin_strip`, `add/remove_cut_cell`, `mark_shared_diffusion` land in `core/atomic_ops.py` on top of the cell-grid + union-find. The `device_resize` macro stops emitting FIN/OD/POLY L1s directly and starts driving them through L2 → CSP delta → decoder.
  - **M4e (Not started).** Cell-level DRC rules for OD / VIA0; CUT-to-gate, CUT-to-via, CUT-to-CUT spacings in `core/drc_constraints.py`. Performance instrumentation on `_propagate` per-layer × track-count goes here too — the roadmap §M4 risk note flags this as the first place engine complexity becomes visible.
- **Files touched.**
  ...
  - `core/grid.py:87-240` (M4b) — `MultiLayerGrid` cell-grid axis.
  - `core/csp_engine.py` (M4b) — net-equivalence union-find; `union` precondition includes "no CUT between adjacent cells"; commit exports net-equivalence delta.
  - `core/drc_constraints.py` (M4e) — cell-level rules for OD / VIA0; CUT spacings.
  - `core/atomic_ops.py` (M4d) — B-tier L2 primitives.
  - `io_adapters/parser.py` (M4c) — FIN / POLY / OD / CUT route through `CellOccupancy` projection; identify shared S/D segments and stamp `shared_with`.
  - `tech/layer_map.py` (M4a) — tier markers + helpers; M4b will add cut-layer GDS numbers as fixtures grow them.
- **Change outline.**
...
- **Acceptance.**
  ...
  - **M4b–e.** Two devices share an OD segment → CSP recognizes a single diffusion region, no conflict. LI laid across two adjacent S/D cells → net-equivalence auto-merges. POLY_CUT inserted mid-span on a full-span poly → net-equivalence splits into two; remove cut → re-merge. Fin removal via cell release → downstream constraints recompute automatically.
...
- **Notes for downstream M4 sub-milestones.**
  - `CellOccupancy.shape_record` is the M3 backlink seam — M4c stamps it on every projected cell so the SKILL/DRC closure in M7 can walk back to the geometric source of truth.
  - The `CellOccupancy.__post_init__` rejection of `OccupantType.WIRE` is the structural guard against silently mis-routing an A-tier layer through the B-tier projection. Loosen it only if M4c finds a justified case.
  - `LAYER_TIER` is the dispatch table M4c keys off; the `test_every_layer_map_entry_has_a_tier` structural test is the regression guard if a future fixture adds a layer without a tier marker.
...
M1, M2, M3, and M4a have landed. The next move is **M4b** — extend `MultiLayerGrid` with a 2D `(track_a, track_b)` cell-grid axis for B-tier layers and grow the CSP engine with an internal net-equivalence union-find (`engine.net_of(cell)` / `engine.connected_cells(net_id)`; `union` precondition includes "no CUT between adjacent cells"; `commit_with_delta` extends to also export a net-equivalence delta). The M4a foundation is in place: `OccupantType.CUT` exists, `CellOccupancy` is defined and ready to slot into the cell-grid, and `LAYER_TIER` / `CUT_LAYERS` / `tier_of()` give the engine a clean dispatch table. M4b's natural feed is still the M3 `shape_pool`: `ShapeRecord`s on B-tier layers will project into `CellOccupancy`s instead of `TrackSegment`s, and unannotated B-tier shapes already carry the `BLOCKAGE` plumbing M3 wired up. Performance instrumentation on `_propagate` per layer × track count belongs in M4b's first PR — incremental net-equivalence over a growing grid is the first place the engine's `_propagate` complexity becomes visible.
