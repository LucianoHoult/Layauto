...
| 1.5. LVS extract (iXref + nXref + NET NAMES + DEVICE INFO + NET SHAPES) | Either spawns `calibre -query <svdb_dir>` and streams `INSTANCE XREF WRITE` / `NET XREF WRITE` / `NET NAMES` / `DEVICE INFO <inst>` / `NET SHAPES <lvs_name>` over stdin (`mode=calibre`) or copies pre-staged `dummy/fixtures/{iXref,nXref}.temp` + `net_names.txt` + `device_info_<inst>.txt` + `net_shapes_<lvs_name>.txt` (`mode=dummy`, default). Parses each format and writes `output/ixref.yaml` (devices), `output/net_xref.yaml` (joined `schematic_name → lvs_name → lvs_index`), `output/device_info.yaml` (per-device `layout_inst → {layer → [bbox_um, ...]}`), and `output/net_shapes.yaml` (per-net `lvs_index/lvs_name/schematic_name → {layer → [bbox_um, ...]}`) as saved-for-later middle files. | `io_adapters/calibre_query.py::extract_ixref`, `extract_net_xref`, `extract_device_info`, `extract_net_shapes` |
| 2.5. Calibre layer overlay | Slice 1.6: walks every A-tier `TrackSegment` and B-tier `CellOccupancy`, looks up the GDS layer's `derived_layers` list in `tech/layer_map.yaml`, finds matching shapes in the parsed `device_info.yaml` / `net_shapes.yaml`, and stamps per-cell `device_id` / `color` (with B-tier diffusion-sharing via `shared_with`). A-tier uses center-point containment; B-tier uses ≥50% area overlap. After the per-cell pass, per-`ShapeRecord` summaries (`net_id`, `device_id`) are re-derived from cell consensus so legacy macro consumers keep working. Runs **alongside** the legacy `apply_lvs_overlay` for now; cutover (legacy delete) is slice 1.6b. | `io_adapters/calibre_layer_map.py::apply_calibre_layer_overlay` |
...
### `device_info_<inst>.txt` and `device_info.yaml`

Calibre HDB ``DEVICE INFO <layout_inst>`` is a per-device query (one subprocess invocation per device); ``layout_inst`` is the LVS device name (`M0`, `M1`, …) carried by `iXref.yaml`'s `devices[*].layout_inst`. The response is stdout-only:

```
Device_Info 20000
Info:
0 0 11 May 07 03:00:00 2026
0
IN
OUT
VSS
...
The `0 0 <n> <timestamp>` line says "n metadata lines follow" — the first is the `<device_type_number>` (0=NMOS, 1=PMOS, …), the next group are `<pin_net>` lines in pin-table order (we don't know the per-device-type pin count; treat as opaque), then `<property_value>` lines (also opaque), an optional `[text_model_name]`, and finally the seed layer name (`ngate_lvt` in this example). After that comes `<a> <b> <c> <date>` (per-shape-group count line) followed by `p <shape_idx> <n_vertices>` and `<n_vertices>` lines of `<y> <x>` integer pairs in `1 / precision` micrometres (for `precision=20000` that's 0.05 nm). Multiple shapes for one layer iterate the `p` blocks; multiple layers append a new layer-name line followed by another count + shape sequence.

Producer: `run_calibre_device_info` spawns `calibre -query <svdb_dir>` with stdin commands `DEVICE INFO <inst>\nEXIT\n`, captures stdout, slices the `Device_Info ... END OF RESPONSE` block via `_extract_device_info_block`, and writes that block to `<out_dir>/device_info_<inst>.txt`. Dummy fixtures live at `dummy/fixtures/device_info_<inst>.txt`; the parametric generator is `generate_calibre_device_info(layout_data, layout_inst, filename)`.
...
- `io_adapters/calibre_layer_map.py` — `load_layer_map_with_derived`, `apply_calibre_layer_overlay`, `LayerOverlayConflictError` (Slice 1.6 per-cell overlay from `tech/layer_map.yaml::derived_layers` + `tech/calibre_layer_map.yaml` registry).
